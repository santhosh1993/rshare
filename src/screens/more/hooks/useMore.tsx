...
export const useMore = () => {
    ...
    const updateUserData = useCallback(async ({name, phoneNo}:{name: string, phoneNo: string}) => {
        try {
            await doc((await userId()).id, FireStoreCollection.USERS).update<FireStoreCollection.USERS>({docData: {
                name: name,
                phoneNo: phoneNo
              }})
    ...
    const validateUserData = useCallback(({name, phoneNo}: {name: string | undefined, phoneNo: string | undefined}) => {
